...
(function(){
    ...
    var assess = {
        ...
        renderPage:function(){
            var _this = this;
            $.get('historyAction!showGroupMembers.action',function(res){// /EnterpriseEvaluateAction!showGroupMembers.action
                _this.groupmap = {};
                _this.analysismap={};
                if(res.code == 0){
                    var groupOpt;
            ...
            $.get('historyAction!showGroupMembers.action',function(res){// /GameGroupManagerAction!showGameGroups.action
                _this.groupmap1 = {};
                if(res.code == 0){
                    var groupOpt;
                    $(res.allGroupMembers).each(function(index,item){
                        groupOpt +='<option value="'+item.groupNames+'">'+item.groupNames+'</option>';
                        _this.groupmap1[item.groupNames] = {
                            name:item.groupNames,
                            num:item.year,
                            period:item.periodOfYears
                        };
                    });
        ...
        groupChange:function(){
            ...
            var itemOpt = '<option value="null">Please select the group</option>'
            for(var i = 1;i <= sum;i++){
                yearOpt += '<option value="'+i+'">num'+i+'year</option>';
            };
        ...
        groupChange1:function(){
            ...
            var peroidOpt = '<option value="null">Please select the period</option>';
            for(var i = 1;i <= peroid;i++){
                peroidOpt += '<option value="'+i+'">num'+i+'period</option>';
            };
        ...
        submitForm:function(){
            ...
            if(group != 'null' && year != 'null' && userunique != 'null'){
                ...
                $.post('historyAction!showEndValue.action',data,function(res){// /EnterpriseEvaluateAction!showEndValue.action
                    if(res.code == 0){
        ...
        submitForm1:function(){
            ...
            if(group != 'null' && year != 'null' && period != 'null'){
                ...
                $.post('historyAction!showEndValues.action',data,function(res){// /enterpriseEvaluateAction!showEndValues.action
                    if(res.code == 0){
                        var data = res.endValues;
                        if(data.length>0){
        ...
        renderChart:function(data){
            ...
            var group = this.$groupSelete.val();
            var year = this.$yearSelete.val();
            this.option.subtext ="game group："+ _this.groupmap[group].name + " num"+year+"year";
            var xAxis =  new Array();
            var series =  new Array();
            $(data).each(function(index,item){
                xAxis.push("num"+item.currentPeriod +"period");
        ...
        renderChart1:function(data){
            ...
            var group = this.$groupSelete1.val();
            var year = this.$yearSelete1.val();
            this.option.subtext ="game group："+ _this.groupmap1[group].name + " num"+year+"year";
            var xAxis =  new Array();
            var series =  new Array();
            $(data).each(function(index,item){
