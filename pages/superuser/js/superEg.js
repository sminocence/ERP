...
(function () {
  var user = {
          ...
          $.extend(true, Table.prototype, {
              setTableData: function () {
                  ...
                  for (var i = curr * option.pageSize; i < res.length && i < option.pageSize + curr * option.pageSize; i++) {
                      ...
                      tr += '<td>' +
                            '<a href="#" class="delete"><span class="icon-trash-empty"></span>cancle</a>' +
                            '<a href="#" class="reset">re-password</a>' +
                            '</td></tr>';
      ...
      deleteItem: function (the) {
          ...
          var data = {'adminId': '' + id};
          DIALOG.confirm('Whether or not the ID is deleted' + id + '、username is' + name + ' User?', function () {
      ...
      resetPassword: function (target) {
          ...
          this.dialog = new DIALOG({
            title: 'confirm <b>' + username + '</b> password is reset to <b>admin </b>？',
          ...
          $('.yes').on('click', function () {
            $.post('superAdminAction!resetAdminPassword.action?rnd='+Math.random(), {
              ...
              if (res.code == 1) {
                ...
                TIP('Reset password failly','error',2000);
              }
            }, 'json');
          });
        }
