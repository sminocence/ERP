(function () {
  ...
  _this.$username.html('hello,' + _this.username + '!');
}
}, 'json');
},
  bindEvent: function () {
  ...
  exitSystem: function () {
  var _this = this;
  DIALOG.confirm('Ok out?', function () {
				// 注销管理员的session
  $.post('superAdminAction!superAdminSignout.action',
					function (res) {
  window.location.replace('../../../erpm/indexeg.html');
});
});
},
  ...
  this.dialog = new DIALOG(
  {
    ...
    content: '<div class="dialog-body" id="modifyPass-wrapper">'
								...
								+ '<label class="label-div"><span class="start">*</span>userID</label>'
								+ '<div class="label-input">'
								+ '<input type="text" id = "adminId" class="form-control" name = "adminId" data-validation="required"'
								+ ' data-validation-error-msg="Please enter the userID" />'
								+ '</div>'
								+ '</div>'
								+ '<div class="input-div clear-fix">'
								...
								+ ' data-validation="required" data-validation-error-msg="please enter username"/>'
								+ '</div>'
								+ '</div>'
								+ '<div class="input-div clear-fix">'
								+ '<label class="label-div"><span class="start">*</span>password</label>'
								+ '<div class="label-input">'
								+ '<input type="password" name="password" class="form-control"'
								+ 'data-validation-error-msg="enter password" data-validation="required"/>'
								+ '</div>'
								+ '</div>'
								+ '</form>'
  ...
  BTN.addLoading(self, 'creating', 'loading');
  ...
  BTN.removeLoading(self, 'create');
  if (res.code == 1) {
  TIP('Create success', 'success', 2000);
  _this.dialog.close();
  window.location.reload();
} else if (res.code == 0) {
  TIP('Create fail', 'error', 2000);
}
}, 'json');
} else{
  TIP('The userID already exists', 'error', 2000);
  BTN.removeLoading(self, 'Re-create');
  AdId.val('');
}
}, 'json');
}
...
(function () {
  ...
  content += '<div class="dialog-footer"><button class="btn j_ok" href="#">confirm</button><button class="btn j_cancel" href="#">cancel</button></div>';
